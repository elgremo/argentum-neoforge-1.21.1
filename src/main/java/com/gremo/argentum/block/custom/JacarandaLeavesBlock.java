...
public class JacarandaLeavesBlock extends LeavesBlock {

    private static final int DROP_INTERVAL = 100;

    public JacarandaLeavesBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        if (level.isClientSide && random.nextInt(6) == 0) {
            ...
            if (!belowState.isSolid() && !belowState.isFaceSturdy(level, belowPos, Direction.UP)) {
                ...
                level.addParticle(ModParticles.JACARANDA_PARTICLES.get(), x, y, z, 0.0, -0.01, 0.0);
            }
        }
    }
}